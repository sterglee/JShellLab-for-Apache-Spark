...
// This is the basic Matrix class of jShellLab.
...
import static jShellLabSci.PrintFormatParams.*;
import static jShellLabSci.math.array.NUMALConvArrays.*;
import jShellLabSci.math.io.files.ASCIIFile;
import java.io.File;
...
final public DoubleStream stream() {
    return Arrays.stream(oneDDoubleArray(d));
}

final public DoubleStream parallelstream() {
    return Arrays.stream(oneDDoubleArray(d)).parallel();
}
...
public Matrix(jShellLabSci.math.array.Vec _a)  {
    d = new double[1][1]; 
...
while (r<sN) {
 ...
 futures[threadId] = GlobalValues.execService.submit(() -> {
     int  a = firstRow;   // the first row of the matrix that this thread processes
        while (a < lastRow) {
            // the last row of the matrix that this thread processes
            int  b = 0;
            while (b < rN) {
                double  s = 0.0;
                int c1 = 0;
                while (c1 < rM) {
                    s += d[b][c1] * thatTrans[a][c1];
                    c1++;
                }
                vr[b][a]   = s;
                b++;
...
/*
    ...
    if (siz  >  jshellLabExec.Interpreter.GlobalValues.mulMultithreadingLimit) 
...
final public static double [] solve(Sparse A, double [] b) {
    return jShellLabSci.math.array.Sparse.sparseSolve(A, b);
}

// solve with the MTJ Column Compressed Matrix
final public static double [] solve(CCMatrix A, double [] b) {
    return jShellLabSci.math.array.CCMatrix.BiCGSolve(A, b);
}
...
final public static double []  solveEJML(double [][]  dA,  double [] db) {
    DMatrixRMaj  drm_dA = new DMatrixRMaj(dA);
    DMatrixRMaj  drm_db = new DMatrixRMaj(db);
    DMatrixRMaj  drm_dx = new DMatrixRMaj(drm_db.getNumRows(), drm_db.getNumCols());
    
    CommonOps_DDRM.solve( drm_dA, drm_db, drm_dx);
    
    return  drm_dx.getData();
}
...
final public static double normInf(double [][] dM) {
    DMatrixRMaj dm = new DMatrixRMaj(dM);
   
    double normInfValue = org.ejml.dense.row.NormOps_DDRM.normPInf(dm);
    return normInfValue;
}

final public static double normInf(Matrix dM) {
   DMatrixRMaj  dm = new DMatrixRMaj(dM.getArray());
      double normInfValue = org.ejml.dense.row.NormOps_DDRM.normPInf(dm);
    return normInfValue;
}
...
final public EigResults  eigMTJ() {  
  EigResults eigA = jShellLabSci.math.array.JILapack.EigMTJ(this.getArray());
  return eigA;
}

final public static EigResults  eigMTJ(Matrix A) {  
  EigResults eigA = jShellLabSci.math.array.JILapack.EigMTJ(A.getArray());
  return eigA;
}

final public static EigResults  eigMTJ(double [][] A) {  
  EigResults eigA = jShellLabSci.math.array.JILapack.EigMTJ(A);
  return eigA;
}
...
final public static double [][]  EJMLDenseMatrixtoDoubleArray(org.ejml.data.DenseD2Matrix64F dm)  {
...
final public static double [][]  EJMLDMatrixRMaj_toDoubleArray(DMatrixRMaj  dm)  {
  double [][]  da = new double [dm.numRows][dm.numCols];
  for (int r=0; r < dm.numRows; r++)
    for (int c=0; c <dm.numCols; c++)
      da[r][c] = dm.get(r,c);
...
final public static Matrix invEJML(Matrix A) {
    DMatrixRMaj da = new DMatrixRMaj (A.getArray());
  org.ejml.dense.row.CommonOps_DDRM.invert(da);
  return new Matrix( EJMLDMatrixRMaj_toDoubleArray(da));
...
final public static double  [] []  eigDEJML(double [][] da) {
    ...
    org.ejml.dense.row.decomposition.eig.SwitchingEigenDecomposition_DDRM  eig_da = new org.ejml.dense.row.decomposition.eig.SwitchingEigenDecomposition_DDRM(dmrm.numRows);
    eig_da.decompose(dmrm);
...
final public static double  [] []  eigVEJML(double [][] da) {
    DMatrixRMaj dm_da = new DMatrixRMaj(da);
    org.ejml.dense.row.decomposition.eig.SwitchingEigenDecomposition_DDRM  eig_da = new org.ejml.dense.row.decomposition.eig.SwitchingEigenDecomposition_DDRM(dm_da.numRows);
    ...
    for (int k=0; k<neigs; k++)  {
        DMatrixRMaj currentEVec = eig_da.getEigenVector(k);
        if (currentEVec != null)
...
final public static double [][]  DGELS(double [][] A, double [][] B) {
    return jShellLabSci.math.array.JILapack.DGELS(A, B);
}

final public static Matrix   DGELS(Matrix A, Matrix B) {
    return new Matrix(jShellLabSci.math.array.JILapack.DGELS(A.getArray(), B.getArray()));
}
...
final public static Matrix EJMLpinv(Matrix A, Matrix invA) {
  org.ejml.data.DenseD2Matrix64F da = new org.ejml.data.DenseD2Matrix64F(A.getArray());
  org.ejml.data.DenseD2Matrix64F dinva = new org.ejml.data.DenseD2Matrix64F(invA.getArray());
...
final public static Matrix EJMLcreateSymmPosDef(int width,  java.util.Random rg)  {
    DenseD2Matrix64F spdDense64 = org.ejml.ops.RandomMatrices.createSymmPosDef(width, rg);
...
/*
  final public static Matrix createSymmetric(int N, double mn, double mx, java.util.Random rg)  {
      ...
      DenseD2Matrix64F dmejml = org.ejml.ops.RandomMatrices.createSymmetric(N,  mn, mx, rg);
...
final public static Matrix createUpperTriangle(int dimen,  int hessenberg, double mn,  double mx, java.util.Random rg ) {
     DenseD2Matrix64F  upperTriangleDense64 = org.ejml.ops.RandomMatrices.createUpperTriangle(dimen, hessenberg, mn, mx, rg);
...
final public void  browse() {
    jshellLabExec.gui.watchMatrix.display(this.getArray());
}

final public void  browse(String matrixName) {
    jshellLabExec.gui.watchMatrix.display(this.getArray(), matrixName);
}
