...
import jshellLabExec.gui.DetailHelpFrame;
import jShellLabSciCommands.FontCellRenderer;
  ...
  public  RSyntaxDocument updateDocument()  {
   ...
   public  String   getSelectedTextOrCurrentLine() {
    ...
    public void keyTyped(KeyEvent e){
       /* int  keyValue = e.getKeyChar();
        ...
        private static void  processListSelection(JList clList) { 
                ...
                jshellLabGlobal.Interpreter.GlobalValues.globalEditorPane.setSelectionStart(GlobalValues.selectionStart);
                jshellLabGlobal.Interpreter.GlobalValues.globalEditorPane.setSelectionEnd(GlobalValues.selectionEnd);
                ...
                jshellLabGlobal.Interpreter.GlobalValues.globalEditorPane.replaceSelection( selected);
        ...
        private static String detectWordAtCursor() {
         ...
         RSyntaxTextArea  editor = jshellLabGlobal.Interpreter.GlobalValues.globalEditorPane;   // get RSyntaxArea based editor instance
      ...
      //   form total word that is under caret position
       ...
       GlobalValues.textForCompletion = wordAtCursor; 
        ...
        public void clickEvalEquations() {
            
                Runnable  execCode = () -> {
         
           java.awt.Cursor  cr =  new java.awt.Cursor(java.awt.Cursor.WAIT_CURSOR);
           GlobalValues.globalEditorPane.setCursor(cr);
       
          String  currentTextForEval = getSelectedTextOrCurrentLine();
         
          GlobalValues.eq.process(currentTextForEval);
          System.out.println("evaluating equation: "+currentTextForEval);
    ...
    System.out.flush();
        ...
        public void clickEvalEquationsResult() {
          ...
          GlobalValues.eq.print(currentTextForEval);
    ...
    System.out.flush();
       ...
       SourceCodeAnalysis sca = jshellLabGlobal.Interpreter.GlobalValues.jshell.sourceCodeAnalysis();
       ...
       do {
        ...
        List<SnippetEvent> grResultSnippets = 
                snippets.stream().map(jshellLabGlobal.Interpreter.GlobalValues.jshell::eval).
   ...
   /*             if (Kind==Snippet.Kind.VAR) {
                ...
                if (vvalue!=null) {
                int MxLen = jshellLabGlobal.Interpreter.GlobalValues.mxLenToDisplay;
                ...
                */
                if (Kind==Snippet.Kind.METHOD) {
                    ...
                    // try to execute as equation
               //     clickEvalEquations();
                }
            }
        
            // check if the currentTextForEval is simply a variable name, if so
            // display its value
            /*
            GlobalValues.jshell.variables().forEach((v) -> {
                
                    String vn = v.name();
                    if (vn.equals(copyOfCurrentTextForEval)) {
                    SysUtils.ConsoleWindow.enable = true;
    ...
    {
        jshellLabEditor.documentEditsPendable = true;
...
clList.addMouseListener(new MouseListener() {
    ...
    Point pp  = jshellLabGlobal.Interpreter.GlobalValues.globalEditorPane.getCaret().getMagicCaretPosition();
     if (pp != null) {
            SwingUtilities.convertPointToScreen(pp, jshellLabGlobal.Interpreter.GlobalValues.globalEditorPane);
            pp.x = pp.x + 2;
            pp.y = pp.y + 20;
    ...
    break;
            ...
            //case KeyEvent.VK_F3:    
               ...
               if (keyValue==KeyEvent.VK_F1) 
                       matches = jshellLabGlobal.Interpreter.GlobalValues.AutoCompletionJShell.getMatched(inputString);
               else 
                       matches = jshellLabGlobal.Interpreter.GlobalValues.AutoCompletionJShell.getMatchedRegEx(inputString);
                ...
                GlobalValues.autoCompletionFrame = new AutoCompletionFrame("JShellLab editor autocompletion, Press F1  for detailed help on the selected entry");
